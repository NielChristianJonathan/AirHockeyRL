...
class AirHockeyGame:
    def __init__(self):
        ...
        self.trajectory_attack  = []
        self.trajectory_defense = []
        ...
        # MODEL
        self.model_defense = PPO.load(
            "defense_ppo_airhockey.zip",
            device="cpu"
        )

        self.model_attack = PPO.load(
    ...
    def get_user_feedback_ab(self, traj_A, traj_B, label=""):
        font_big   = pygame.font.SysFont("Calibri", 42)
        font_small = pygame.font.SysFont("Calibri", 30)

        reward_A = sum(s["reward"] for s in traj_A)
        reward_B = sum(s["reward"] for s in traj_B)
        ...
        while waiting:
            ...
            title = font_big.render(
                f"[{label}] Which episode was BETTER?",
                True, (255, 255, 100)
            )
            self.screen.blit(title, title.get_rect(
                centerx=self.screen_width // 2, y=60
            ))

            # Panel A
            panel_w, panel_h = 420, 220
            gap    = 60
            left_x = (self.screen_width - (panel_w * 2 + gap)) // 2
            panel_y = 150

            panel_A = pygame.Rect(left_x, panel_y, panel_w, panel_h)
            panel_B = pygame.Rect(left_x + panel_w + gap, panel_y, panel_w, panel_h)

            pygame.draw.rect(self.screen, (0, 80, 160), panel_A, border_radius=12)
            pygame.draw.rect(self.screen, (160, 0, 0),  panel_B, border_radius=12)
            pygame.draw.rect(self.screen, (255,255,255), panel_A, 2, border_radius=12)
            pygame.draw.rect(self.screen, (255,255,255), panel_B, 2, border_radius=12)

            def draw_panel(label_ep, reward, steps, rect, key):
                lines = [
                    (label_ep,                        font_big,   (255,255,100)),
                    (f"Total Reward : {reward:.2f}",  font_small, (255,255,255)),
                    (f"Steps        : {steps}",        font_small, (255,255,255)),
                    (f"Press [{key}] to choose",       font_small, (180,255,180)),
                ]
                y = rect.y + 28
            ...
            draw_panel("Game Lalu (A)", reward_A, len(traj_A), panel_A, "A")
            draw_panel("Game Ini  (B)", reward_B, len(traj_B), panel_B, "B")

            skip = font_small.render("Press [S] to skip", True, (150,150,150))
            self.screen.blit(skip, skip.get_rect(
                centerx=self.screen_width // 2, y=panel_y + panel_h + 30
            ))
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    elif event.key == pygame.K_s: return None

    def save_preference(self, traj_A, traj_B, preferred, mode):
        data = {
            "traj_A":    traj_A[-150:],
            "traj_B":    traj_B[-150:],
            "preferred": preferred,
        ...
        filename = f"preference_{mode}.jsonl"
        with open(filename, "a") as f:
            f.write(json.dumps(data) + "\n")
        print(f"[RLHF] '{preferred}' disimpan ke {filename}")


    def run(self):
        pygame.mouse.set_visible(False)
        running = True
        last_trajectory = None  # ← simpan trajectory game sebelumnya

        while running:
            if self.state == "game_over" and not self.feedback_given:

                # ── Defense ──
                if len(self.trajectory_defense) > 20:
                    if last_trajectory_defense is not None:
                        preferred = self.get_user_feedback_ab(
                            last_trajectory_defense,
                            self.trajectory_defense,
                            label="DEFENSE"
                        )
                    ...
                    last_trajectory_defense = self.trajectory_defense

                # ── Attack ──
                if len(self.trajectory_attack) > 20:
                    if last_trajectory_attack is not None:
                        preferred = self.get_user_feedback_ab(
                            last_trajectory_attack,
                            self.trajectory_attack,
                            label="ATTACK"
                        )
                        if preferred is not None:
                    ...
                    last_trajectory_attack = self.trajectory_attack
            ...
            if self.state == "playing":
                ...
                obs, reward, done, _, _ = self.env.step(ai_action, self.mode, None)

                step_data = {
                    "state":  self.obs.tolist(),
                    "action": ai_action.tolist(),
                    "reward": float(reward),
                    "mode":   self.mode
                }

                # ← Simpan ke trajectory sesuai mode aktif
                if self.mode == "attack":
                    self.trajectory_attack.append(step_data)
                else:
                    self.trajectory_defense.append(step_data)
    ...
    def collect_episode_silent(self, max_steps=500):
        """
        Jalankan 1 episode TANPA render → untuk kumpulkan trajektori A atau B.
        Pakai env yang sama tapi tanpa update display.
        """
        self.env.reset()
        ...
        for _ in range(max_steps):
            action = self.ai_model_control(obs)
            obs, reward, done, _, _ = self.env.step(action, self.mode, None)
    ...
    def draw_game_over(self, screen):
        ...
        screen.blit(win_text, win_text.get_rect(center=(width//2, panel_rect.top + 140)))

        # SCORE
        score_text = f"{self.score_player}  -  {self.score_ai}"
        score_render = font_score.render(score_text, True, (200, 200, 200))
        screen.blit(score_render, score_render.get_rect(center=(width//2, panel_rect.top + 200)))
        ...
        buttons = [
            ("Restart", panel_rect.top + 280),
            ("Menu", panel_rect.top + 350)
        ]
